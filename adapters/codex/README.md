...
`~/.codex/skills/xiaotao/`，并以官方用户级 Skill 目录 `~/.agents/skills/xiaotao/` 作为兜底。
...
| 只有用户级 / 全局 Core，项目内没有 Core | Hook 从 `~/.codex/skills/xiaotao/` 或 `~/.agents/skills/xiaotao/` 调用 Catalog，并输出项目 `.xiaotao/` 的真实状态总览 |
