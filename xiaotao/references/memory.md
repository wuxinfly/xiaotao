...
XiaoTao 的记忆用于延续协作与沉淀经验，不用于复刻对话或流水账执行日志。

## 长期记忆三层架构（Three-Tier Long-Term Architecture）

针对长期保存的内容，XiaoTao 将其清晰划分为**全局用户记忆**、**项目记忆**与**项目知识库**三处，并提供有界逐层索引与独立路由：
...
2. **权威文件不替代**：Temporary 与 Task 仍是进行中工作和原始执行记录；Playbook 仍承载可复用流程步骤；项目记忆与知识库不替代这些底层权威文件。
3. **逐层有界下钻**：
   - **项目记忆**：按“项目总览 → 年度总览 → 月度摘要 → 每日事件 → 对应 Task/Temporary、决定及来源”下钻。月份或年份内容增多时重写上层摘要，保留日期层及原始细节。用户问“我们经历了什么”时先讲主线，追问时定位具体事件。
   - **项目知识库**：按“项目总览 → 主题/标签 → 当前说明 → 来源与历史”下钻，随项目变化修订现行内容，不把时间线当作当前事实。
...
4. **长期记忆入库与路由规则**：
   ...
   - **评审与不可变决策**：写入前检查来源、重复、冲突与现行证据，严格沿用 Memory Worker 提案（只读隔离、禁止自我批准）与 Decision Record；无冲突的显式创建继续保持轻量授权路径。
...
### 待审任务的长期记忆审查（Pending Task Review）

为保证业务任务收尾与长期记忆提炼解耦，Task 归档后的记忆审查按需独立进行：
